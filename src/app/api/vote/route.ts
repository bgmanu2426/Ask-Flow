...
export async function POST(request: NextRequest) {
    try {
        ...
        const voteList = await databases.listDocuments(db, voteCollection, [
            Query.equal("type", type),
            Query.equal("typeId", typeId),
            Query.equal("votedById", votedById),
        ]);

        if (voteList.documents.length > 0) {
            await databases.deleteDocument(db, voteCollection, voteList.documents[0].$id);
            ...
            await users.updatePrefs<UserPrefs>(questionOrAnswer.authorId, {
                reputation:
                    voteList.documents[0].voteStatus === "upvoted"
        ...
        if (voteList.documents[0]?.voteStatus !== voteStatus) {
            ...
            return NextResponse.json(
                {
                    ...
                    message: voteList.documents[0] ? "Vote Status Updated" : "Voted",
                },
                {
                    status: 201,
                }
